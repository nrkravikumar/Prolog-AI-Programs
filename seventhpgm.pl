...
estimation(Start,Final,E), astar1([(E,E,0,[Start])],Final,_,Tp).
...
findall((NewSum,E1,NP,[Z,X|R1]),(street(X,Z,V),
                                 not(member(Z,R1)),NP is P+V,estimation(Z,Final,E1), NewSum is E1+NP),L),append(R2,L,R3),
sort(R3,R4), astar1(R4,Final,C,Tp).
...
%Output: Here we have to click on next button until we 
		have to get either True or False.
